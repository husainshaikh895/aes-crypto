...
class AesCrypto {
    ...
    if (!AesCrypto.cache.has(secretKey)) {
        AesCrypto.cache.set(secretKey, Buffer.from(secretKey, 'hex'));
    }
        return AesCrypto.cache.get(secretKey);
    }
  ...
  static encrypt(textPlain, secretKey) {
    ...
    const secretBuffer = AesCrypto.getSecretBuffer(secretKey);
